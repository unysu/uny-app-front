...
abstract class UnyAPI extends ChopperService {
  ...
  Future<Response> addInterests(
      @Header('Authorization') String token, @Body() var data);

  @Post(path: '/user/update_user')
  Future<Response> editAboutMe(
      @Header('Authorization') String token, @Body() var data);

  @Post(path: '/user/update_media')
  Future<Response> updateMedia(
      @Header('Authorization') String token, @Body() var data);
  ...
  @Post(path: '/user/update_user')
  Future<Response<UserDataModel>> updateUser(
      @Header('Authorization') String token, @Body() var data);
  ...
  Future<Response<AllChatsModel>> getAllChats(
      @Header('Authorization') String token, @Body() var data);

  @Post(path: '/user/get_message_by_chat_room_id')
  Future<Response<RoomMessagesModel>> getRoomMessages(
      @Header('Authorization') String token,
      @Query('chat_room_id') int id,
      @Query('older_from') String date);
  ...
  Future<Response<Message>> editMessage(
      @Header('Authorization') String token, @Body() var data);

  @Post(path: '/user/remove_message')
  Future<Response<Message>> deleteMessage(
      @Header('Authorization') String token, @Body() var data);

  @Post(path: '/user/remove_chat_room_messages')
  Future<Response<Message>> clearChat(
      @Header('Authorization') String token, @Body() var data);
  ...
  Future<Response<Matches>> getUserById(
  ...
  Future<Response<AllUserDataModel>> getCurrentUser(
      @Header('Authorization') String token);

  @Get(path: '/user/get_all_media')
  Future<Response<MediaDataModel>> getMedia(
      @Header('Authorization') String token);
  ...
  static UnyAPI create(String converterCode) {
    ...
    switch (converterCode) {
      ...
      case Constants.SIMPLE_MESSAGE_CONVERTER:
        converter =
            JsonToTypeConverter({Message: (json) => Message.fromJson(json)});
        break;
      case Constants.MATCHES_JSON_CONVERTER:
        converter =
            JsonToTypeConverter({Matches: (json) => Matches.fromJson(json)});
        break;
    }

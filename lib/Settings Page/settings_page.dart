...
class _SettingsPageState extends State<SettingsPage> {
    ...
    return Column(
      children: [
          ...
          child: Stack(
            children: [
              GestureDetector(
                child: Container(
                  height: 150,
                  child: Center(child: Consumer<UserDataProvider>(
                    builder: (context, viewModel, child) {
                      if (viewModel.mediaDataModel!.mainPhoto != null) {
                        profilePictureUrl =
                            viewModel.mediaDataModel!.mainPhoto!.url;
        ...
        Consumer<UserDataProvider>(
          builder: (context, viewModel, child) {
            return Text(
                '${viewModel.userDataModel!.firstName} ${viewModel.userDataModel!.lastName}  ${DateTime.now().year - (int.parse(viewModel.userDataModel!.dateOfBirth.split('-')[0]))}',
        ...
        Consumer<UserDataProvider>(
          builder: (context, viewModel, child) {
            String _phoneNumber = viewModel.userDataModel!.phone;
            String _russianFormattedNumber = _phoneNumber[0] +
                _phoneNumber[1] +
                ' ' +
                '(' +
                _phoneNumber[2] +
            ...
            return Text(_russianFormattedNumber,
        ...
        ListTile(
            contentPadding:
                EdgeInsets.only(left: width / 21, right: width / 20),
        ...
        ListTile(
            contentPadding:
                EdgeInsets.only(left: width / 23, right: width / 20),
            ...
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => BlockedUsersSettingsPage()));
            }),
        ListTile(
            contentPadding:
                EdgeInsets.only(left: width / 20, right: width / 20),
            title: Text('Техподдержка', style: TextStyle(fontSize: 17)),
            leading: SvgPicture.asset(_assistanceAsset,
                color: AdaptiveTheme.of(context).mode == AdaptiveThemeMode.light
                    ? Color.fromRGBO(145, 10, 251, 5)
                    : Colors.purpleAccent),
            trailing: Icon(CupertinoIcons.forward),
        ...
        ListTile(
          ...
          title:
              Text('Выйти', style: TextStyle(fontSize: 17, color: Colors.red)),
        ...
        child: Column(
          children: [
            ...
            ListTile(
                ...
                onTap: () async {
                  if (_mainPhotoId != null) {
                    ...
                    var data = {'media_id': _mainPhotoId};

                    await UnyAPI.create(Constants.SIMPLE_RESPONSE_CONVERTER)
                        .deleteMedia(token, data)
                        .whenComplete(() async {
                      await UnyAPI.create(
                              Constants.ALL_USER_DATA_MODEL_CONVERTER_CONSTANT)
            ...
            ListTile(
              ...
              leading:
                  SvgPicture.asset(_newMediaImageAsset, color: Colors.grey),
              trailing: Icon(Icons.arrow_forward_ios_rounded, size: 20),
              onTap: () async {
                loadingBarState!(() {
                  _showLoading = true;
          ...
          builder: (context) {
            return CupertinoActionSheet(
              ...
              cancelButton: CupertinoActionSheetAction(
                onPressed: () => Navigator.pop(context),
                child:
                    Text('Отмена', style: TextStyle(color: Colors.lightBlue)),
              ),
            );
          });
    } else if (UniversalPlatform.isAndroid) {
  ...
  void _cropImage(String? filePath) async {
    ...
    var data = {'media': base64Img, 'mime': mime, 'filter': 'main+'};
